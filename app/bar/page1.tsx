...
export default function BarPage() {
  return (
    <div className="bg-black min-h-screen p-6 md:p-20 text-white pb-40">
      {/* 1. THE HEADER */}
      <header className="mb-20">
        <motion.p 
          initial={{ opacity: 0, x: -10 }}
          animate={{ opacity: 1, x: 0 }}
          className="text-[#A8B475] font-bold tracking-[0.5em] text-[10px] uppercase mb-4"
        >
          the bar & lounge
        </motion.p>
        <h1 className="text-6xl md:text-8xl font-serif italic leading-none">
          Vibe & <span className="text-white/20 underline decoration-[#A8B475] underline-offset-8 transition-all hover:text-white">Flow.</span>
        </h1>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
        ...
        {/* BOX A: COCKTAILS (The main focus) */}
        <motion.div 
          whileHover={{ y: -5 }}
        ...
        >
          <div>
            <div className="flex justify-between items-center mb-16">
              <h2 className="text-4xl font-serif italic text-[#A8B475]">Signature Cocktails</h2>
              <GlassWater className="text-[#A8B475]" size={24} />
            </div>
            <div className="space-y-10">
              {cocktails.map((item) => (
                <div key={item.name} className="group cursor-default">
                  <div className="flex justify-between items-baseline">
                    <h4 className="font-bold tracking-[0.2em] uppercase text-xs group-hover:text-[#A8B475] transition-colors">{item.name}</h4>
        ...
        {/* BOX B: BEERS & WINE (Clean Jus Green Box) */}
