  ...
  return (
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 w-full h-20 bg-white border-b border-stone-100 z-[100] px-6 flex justify-between items-center shadow-sm">
        ...
        >
          <motion.div animate={isOpen ? { rotate: 45, y: 4 } : { rotate: 0, y: 0 }} className="w-6 h-[1.5px] bg-[#1B4D3E]" />
          <motion.div animate={isOpen ? { rotate: -45, y: -4 } : { rotate: 0, y: 0 }} className="w-6 h-[1.5px] bg-[#1B4D3E]" />
        </button>
      </header>
          ...
          >
            <nav className="flex-1 flex flex-col items-center justify-center gap-8 pt-20">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-4xl font-serif italic tracking-tighter ${pathname === link.href ? "text-[#A8B475]" : "text-[#1B4D3E]"}`}
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="p-12 border-t border-stone-50 bg-[#fdfcf8] flex flex-col items-center gap-8">
               <div className="flex gap-12 items-center">
                  <a href="https://google.com/maps" target="_blank" className="text-[#1B4D3E]"><MapPin size={24} /></a>
                  <a href="https://www.instagram.com/jusfishy_and_beyond/" target="_blank" className="text-[#1B4D3E]"><Instagram size={24} /></a>
               </div>
               <Link href="https://tywebstudio.com" target="_blank" className="group flex flex-col items-center gap-1">
                  <span className="text-[7px] font-black tracking-[0.4em] text-stone-500">DIGITAL ARCHITECTURE BY</span>
                  <span className="text-[10px] font-black tracking-[0.2em] text-[#1B4D3E] border-b border-[#A8B475]">TYWEBSTUDIO.COM</span>
               </Link>
            </div>
          </motion.div>
        )}
      ...
      <header className="hidden md:flex w-20 h-screen sticky top-0 flex-col items-center justify-between py-12 border-r border-stone-100 bg-white z-50">
        <Link href="/" className="group">
          <div className="w-12 h-12 rounded-full bg-[#1B4D3E] flex items-center justify-center overflow-hidden group-hover:scale-110 transition-transform shadow-md">
            <Image src="/jus-fishy-seafood-restaurant-brooklyn-flatbush.jpg" alt="Logo" width={50} height={50} className="object-cover" />
          </div>
        </Link>
        ...
        <nav className="flex flex-col gap-12 flex-1 justify-center">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.name} href={link.href} className="relative group flex items-center justify-center">
                <span 
                  className={`text-[10px] tracking-[0.3em] font-bold transition-colors uppercase 
                  ${isActive ? "text-[#1B4D3E]" : "text-stone-600 group-hover:text-black"}`}
                  style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}
                >
                  {link.name}
                </span>
                {isActive && (
                  <motion.div layoutId="sidebar-pill" className="absolute inset-x-[-12px] inset-y-[-15px] border-[1.5px] border-[#1B4D3E] rounded-full" />
                )}
              </Link>
            );
          })}
        ...
        <div className="flex flex-col items-center gap-10">
          <a href="https://google.com/maps" target="_blank" className="p-2 rounded-full border border-stone-50 hover:bg-[#1B4D3E] hover:text-white transition-all text-stone-500">
            <MapPin size={16} />
          </a>
          <Link href="https://www.instagram.com/jusfishy_and_beyond/" target="_blank">
            <Instagram size={18} className="text-stone-500 hover:text-[#1B4D3E] transition-colors" />
          </Link>
        </div>
      </header>
    </>
